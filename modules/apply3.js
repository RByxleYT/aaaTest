      ...
      try {
        ...
        if (cooldown.has(user.id)) {
          return user.send(new Discord.MessageEmbed()
            ...
            .addField("Why is there a delay?", "Because its for preventing spamming applications")
          )
        } else {
        ...
        function ask_question(qu) {
          ...
          user.send(new Discord.MessageEmbed()
              .setColor("#7883ff")
              .setDescription(qu)
              .setAuthor(`Question ${counter + 1} / ${Questions.length}`, client.user.displayAvatarURL(), "https://discord.com/api/oauth2/authorize?client_id=878078582423953448&permissions=8&scope=bot")
              .setFooter(`${message.guild.name}`, message.guild.iconURL({
                dynamic: true
              }))
            ).then(msg => {
            ...
            user.send(new Discord.MessageEmbed()
              ...
              .then(async collected => {
                ...
                if(reaction.emoji.name === "✅"){
                  ...
                  var embed = new Discord.MessageEmbed().setFooter(`${message.guild.name}`, message.guild.iconURL({
                      dynamic: true
                    }))
                    .setColor("#7883ff")
                    
                    .setTitle("A new application from: `" + originaluser.tag + "`") //${user.tag} -
                    .setDescription(`▶️️ ${originaluser}  |  \`${moment().format().split("-")[2].split("T")[0] + "/" + moment().format().split("-")[1] + "/" + moment().format().split("-")[0]+ " | " + moment().format().split("T")[1]}\``)
                  ...
                  try {
                    ...
                    /* */ 
                      ...
                      .addField("\u200b", `▶️️ Go Back to the Channel ${originalchannel}\n\n▶️️ Use [Scroller](https://discord.com/api/oauth2/authorize?client_id=878078582423953448&permissions=8&scope=bot) in Your Server`).setFooter(`${message.guild.name}`, message.guild.iconURL({
                        dynamic: true
                      }))
    ...
    if (message.channel.id === apply_db.get(message.guild.id, "f_channel_id") && (all_finished_reactions.includes(reaction.emoji.name))) {
      try {
        ...
        if (reaction.emoji.name === "✅") {
          ...
          var approve = new Discord.MessageEmbed().setFooter(`${message.guild.name}`, message.guild.iconURL({
              dynamic: true
            }))
            .setColor("GREEN")
        ...
        if (reaction.emoji.name === "❌") {
          embed.setColor("RED")
          targetMessage.edit(embed)
          var deny = new Discord.MessageEmbed().setFooter(`${message.guild.name}`, message.guild.iconURL({
              dynamic: true
            }))
            .setColor("RED")
